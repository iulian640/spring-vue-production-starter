 ...
 * en producción SIEMPRE por variable de entorno (SOFRITO_SEGURIDAD_JWT_SECRETO),
...
public class JwtConfig {
    ...
    @Bean
    public JwtEncoder jwtEncoder(@Value("${sofrito.seguridad.jwt.secreto}") String secreto,
                                 Environment entorno) {
        return new NimbusJwtEncoder(new ImmutableSecret<>(clave(secreto, entorno)));
    }

    @Bean
    public JwtDecoder jwtDecoder(@Value("${sofrito.seguridad.jwt.secreto}") String secreto,
                                 Environment entorno) {
        ...
        if (!perfilNoProductivo && SECRETOS_DE_JUGUETE.contains(secreto)) {
            throw new IllegalStateException(
                    "Secreto JWT de desarrollo detectado fuera de dev/local/test: configura "
                            + "SOFRITO_SEGURIDAD_JWT_SECRETO con un secreto real (p. ej. openssl rand -base64 48)");
        }
